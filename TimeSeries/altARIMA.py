...
def prepare_data(df):
    ...
    df['Close'] = df['Close'].ffill()  # Forward fill any missing values
    return df
...
def arima_model_and_forecast(df, ticker, forecast_steps=30):
    ...
    if order is None:
        print(f"Could not find suitable ARIMA parameters for {ticker}")
        return None, None
    ...
    try:
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'data/pics/{ticker}_arima_forecast_alt.pdf')
        plt.close()
        
        return results, future_forecast
    except Exception as e:
        print(f"Error fitting ARIMA model for {ticker}: {str(e)}")
        return None, None
    ...
    for ticker in tickers:
        try:
            ...
            if forecast is not None:
                ...
                print(f"Could not generate forecast for {ticker}")
        except Exception as e:
